...
The taxonomy shows visible areas and gaps. In the current live local Figure 2 bundle, 11 categories are authority-backed, 1 is candidate-only, and 3 remain explicit zero-seed local gaps. Security risk and agentic-control cases are visible in the reviewed core. Public-sector, enforcement, identity, education, and procedural categories are present but uneven. Before reusing this paragraph or its companion figure in live manuscript, dashboard, reviewer-response, or thesis prose, route the exact Figure 2 / Table 2 question through `results/boost/limen-boost-046/figure2-question-router.tsv` so the live `35 / 27` denominator, typed zero-seed semantics, residual guardrail logic, and overlay-versus-subtype cautions travel together.

![The heatmap shows support and gaps; it does not rank harm or legality.](../figures/figure5-taxonomy-heatmap.png){ width=100% }

The zero-seed rows require typed interpretation rather than one flat reading. Health remains a true local zero in the reviewed package. Research-integrity remains zero-seed in headline counts but locally non-empty through the source-depth panel, so it is a routing and evidence-ceiling issue rather than a clean absence claim. Residual remains a methods guardrail for cases that should not be forced into a premature label. The large discovery layer therefore supplies future candidates without treating every signal as a reviewed case or every zero-seed row as evidence of real-world absence.
...
Negative results are part of that source-depth method rather than a separate embarrassment. Open agency summaries can still sit below court-text depth; blocked publisher-notice pages can still leave only metadata-grade retraction evidence; public-sector register rows can still remain below exact procurement linkage; and accusation-stage or summary-only records should not be paraphrased as if they were final adjudications. The ladder lets the dashboard and manuscript use large coverage without laundering weak or partial evidence into stronger claims, and it should travel with the negative-results table and the source-depth panels rather than stand alone as a generic maturity graphic.
